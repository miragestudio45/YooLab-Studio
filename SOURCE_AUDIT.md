...
| Assets imported | **Open Industry: yes** (two GLBs, six textures). **Quadrotor sandbox: no** |
...
and they ship: `Six-Axis_01.glb` and `EOAT_Suction.glb`, with the notice beside
them at `public/asset/practice/robot/LICENSE` and the modifications itemised in
`THIRD_PARTY_ASSETS.md`. The joint rig is not in the GLB — the file is nine loose
meshes — so it is transcribed from `parts/SixAxisRobot.tscn` into
`app/lib/robot/sixAxis.ts`, pivot by pivot, together with the joint-limit table
and the home pose from `six_axis_robot.gd`.
...
**The quadrotor sandbox.** Same MIT licence, opposite conclusion on art, and the
difference is not the licence — it is *where the files are*. Its aircraft, city,
props, audio and panorama are not in the repository; they are fetched at runtime
from `cdn.mint.gg`, generated by a separate product (Mint MCP), and a repository's
licence does not grant rights over files it does not contain. Nobody has stated
terms for those artifacts, so nothing was downloaded. What was taken is code, and
more of it than before: the flight core (already shipped), plus the four-mode
camera, the pose interpolation the onboard view depends on, the rotor-disc blur
and the motion trail. What was left behind: Rapier (1.1 MB of WASM to answer a
question a dozen box tests answer), the 32-ray lidar and occupancy grid, the
seven-aircraft roster, the tuning panel and the plots.

So the drone's airframe and its flight hall are still built from Three.js
primitives — and the airframe is now genuinely modelled rather than the thirty
boxes the licence position was previously used to excuse. The chase camera sits
1.1 m behind it and the onboard camera is *inside* it; at that range a
placeholder is not a compromise, it is the whole picture.
...
| Thực hành & STEM | 3 live | YooLab Formula workshop; quadrotor flight core adapted from the MIT sandbox with no art taken; Open Industry six-axis arm shipped under MIT with its rig transcribed |

Two of the four candidate cell meshes were rejected on licence grounds, one
aircraft pack was rejected on the same grounds while its code was adopted, and
two subjects ship empty. Both outcomes are deliberate: a stated gap costs a visitor
